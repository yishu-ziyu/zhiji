# FC-OPC iBot vNext：产品与工程优化 PRD

> 文档类型：现状审计 + 产品需求文档（PRD）+ 目标架构 + 技术重构路线图 + 团队实施规范
> 审计对象：`yishu-ziyu/fc-opc-ibot`
...
这份文档不是“功能愿望池”，而是把 FC-OPC iBot 从高质量原型推进到可持续产品的工程合同。
...
FC-OPC iBot 已经不是“聊天框套模型”。它已经有一套值得继续投入的产品骨架：
...
**FC-OPC iBot 是一个本地优先的项目理解工作台：它在用户授权的项目文件夹内读取真实材料，形成可追溯的候选理解与下一步，并在用户确认后沉淀为可继续工作的项目记忆。**
...
```json
{
  "selection": {
    "id": "sel_...",
    "folderName": "fc-opc-ibot",
    "displayPath": "/Users/.../fc-opc-ibot",
    "expiresAt": "..."
  }
}
```
...
FC-OPC iBot 最有价值的部分，不是画布、八步动画或某个模型，而是它已经抓住了 Agent 产品中最难的一件事：
...
做到这些，FC-OPC iBot 才会从一个“设计思想很先进、实现中仍有捷径的强原型”，升级为一款开发团队可以放心迭代、用户可以逐步托付真实项目的 Agent 产品。
