...
export default function EcommercePage() {
  ...
  const handleSend = useCallback(async (content: string) => {
    ...
    try {
      const endpoint =
        mode === "analyze" ? "/api/ecommerce/analyze" : "/api/ecommerce/script";

      const res = await fetch(endpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(mode === "analyze" ? { productName: content } : { productName: content }),
      });
      ...
      const aiMsg: Message = {
        ...
        content: "",
        timestamp: new Date(),
        type: mode === "analyze" ? "analysis" : "script",
        data,
      };
