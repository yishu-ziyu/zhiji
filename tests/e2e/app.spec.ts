...
test.describe("API Endpoints", () => {
  ...
  test("analyze endpoint validates empty input", async ({ request }) => {
    const res = await request.post(`${BASE_URL}/api/ecommerce/analyze`, { data: {} });
    expect(res.status()).toBe(400);
  });

  test("minutes endpoint validates empty input", async ({ request }) => {
...
test.describe("LLM Integration", () => {
  test("analyze returns structured data", async ({ request }) => {
    const res = await request.post(`${BASE_URL}/api/ecommerce/analyze`, {
      data: { productName: "无线蓝牙耳机" },
    });
    expect(res.ok()).toBe(true);
  ...
  test("script returns data for a product", async ({ request }) => {
    const res = await request.post(`${BASE_URL}/api/ecommerce/script`, {
      data: { productName: "便携充电宝" },
    });
    expect(res.ok()).toBe(true);
    const data = await res.json();
  ...
  test("minutes returns structured data", async ({ request }) => {
    const res = await request.post(`${BASE_URL}/api/efficiency/minutes`, {
      data: { transcript: "今天决定产品7月15日上线" },
    });
    expect(res.ok()).toBe(true);
    const data = await res.json();
