...
**知识足迹（检索点亮）规格：** 见 `docs/SPEC-knowledge-footprint.md`（先规格后实现）。
...
**做：** 资料检索 + 依据卡片 + 工作项（状态、负责人、下一步、时间线）。

页面：`/track/knowledge`

一句话：

> 搜得到、卡片带来源、工作项看得见谁和下一步、时间线能写回。
...
2. 搜「检索 来源」→ 看到带来源的卡片
3. 右侧工作项：看负责人与下一步；点进详情看时间线
4. 写一条评论 → 时间线出现；再推进状态
5. （可选）把检索到的卡挂到工作项上
6. 对评委说：搜得到、有来源、谁负责、下一步是什么、沟通过程在板上。
