...
**当前项目画布开发进度：** 见 `WORKFLOW_STATUS.md`（阶段、产物、验证和风险的唯一进度记录）。
**当前项目画布交互规格：** 见 `.ship/tasks/knowledge-mainline-20260714-174246/plan/spec.md` 的 2026-07-15 修订（独立复核完成后定稿）。
...
**做：** 资料检索、依据卡片、工作项、事件和关系仍是数据基础；用户看到的产品正在改为“项目画布”：Agent 主动指出当前值得关注的对象，用户点击后只展开一层直接关系，右侧解释依据和影响，底部显示相关时间线。

页面：`/track/knowledge`

一句话：

> 重新进入项目时，不必靠记忆寻找进度：系统指出当前重点及依据，用户沿直接关系理解情况、作出决定，并继续追踪人或 Agent 的执行结果。
...
5. 当前视觉基准是 `docs/design/project-canvas-selected.png`；确认记录见 `docs/design/project-canvas-visual-approval.md`。保持其 Apple 式白色桌面布局，颜色只表达业务状态。
6. 四个连续界面状态已经确认，按同一页面内的真实交互实现。
7. 有单测；浏览器里要能走通演示步骤。
...
这组步骤用于防止已有能力回退，不再定义改版后的最终演示。最终演示以项目画布的四个连续状态和实现计划为准。
